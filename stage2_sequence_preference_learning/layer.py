...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.add_lora:
            # self.query_lora = LoRALayer(hidden_size, lora_rank)
            self.key_lora = LoRALayer(hidden_size, lora_rank)
            self.value_lora = LoRALayer(hidden_size, lora_rank)
...
class CrossMultiHeadAttention(nn.Module):
    ...
    def forward(self, query_tensor, context_tensor, attention_mask):
        mixed_query_layer = self.query(query_tensor)
        mixed_key_layer = self.key(context_tensor)
        mixed_value_layer = self.value(context_tensor)
...
class FeedForward(nn.Module):
    ...
    def __init__(
        self, hidden_size, inner_size, hidden_dropout_prob, hidden_act, layer_norm_eps, add_lora=False, lora_rank=4
    ):
        ...
        self.add_lora = add_lora
        if self.add_lora:
            self.dense_1_lora = LoRALayer(hidden_size, lora_rank)
            self.dense_2_lora = LoRALayer(hidden_size, lora_rank)
    ...
    def forward(self, input_tensor):
        ...
        if self.add_lora:
            hidden_states = self.dense_1_lora(hidden_states)

        hidden_states = self.dense_2(hidden_states)
        hidden_states = self.dropout(hidden_states)
        if self.add_lora:
            hidden_states = self.dense_2_lora(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
...
class LoRALayer(nn.Module):
    def __init__(self, in_features, rank=4, alpha=1.0):
        ...
        # Apply LoRA-specific initialization
...
class TransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        # # Add LoRA layer if specified
        self.add_lora = add_lora
        if self.add_lora:
            self.att_lora_layer = LoRALayer(hidden_size, lora_rank)
            self.ffw_lora_layer = LoRALayer(hidden_size, lora_rank)

    def forward(self, hidden_states, attention_mask):
        attention_output = self.multi_head_attention(hidden_states, attention_mask)
        if self.add_lora:
            attention_output = self.att_lora_layer(attention_output)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, query_tensor, content_tensor, cross_attention_mask):
        # Cross Attention
        cross_attention_output = self.cross_attention(query_tensor, content_tensor, cross_attention_mask)
        # Feed Forward
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_tensor_query, input_tensor_content, cross_attention_mask, output_all_encoded_layers=True):
        all_decoder_layers = []
